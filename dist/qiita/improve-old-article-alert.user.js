...
// @version      0.1.0
// @description  Qiitaの古い記事のアラートを改善します。
...
(function (dayjs) {
    ...
    const findArticle = () => {
        ...
        return jsonLdElements
            .map(({ textContent }) => textContent !== null ? JSON.parse(textContent) : undefined)
            .find(isArticle);
    };
    ...
    const updateAlert = () => {
        ...
        const { textContent: currentText } = alertTextElement;
        if (currentText === null) {
            return;
        }
        ...
        const newText = currentText.replace(/この記事は最終更新日から\d+年以上が経過しています。/, `この記事は最終更新日から${year}年以上が経過しています。`);
        alertTextElement.textContent = newText;
    };
